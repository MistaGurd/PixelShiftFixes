# Denne kode stammer fra gruppens eksamensprojekt i programmering.
...
class PixelWipe(Screen): # Hovedklasse, som matcher med klassen i kivy koden
    ...
    def __init__(self, **kwargs):  # Standard-kaldbare-attributes defineres her
        ...
        self.default_output_folder = os.path.join(os.path.expanduser("~"),
                                                  "Downloads")  # Når man behandler et billede (el. mappe)
    ...
    def on_drop(self, window, file_path):  # Metode, som håndterer drag-and-drop
        ...
        if os.path.isdir(path):  # Hvis det er en mappe (dir for directory/mappe)
            ...
            self.update_file_info(self.selected_path,
                                  "Output: Ikke valgt")  # Standardtekst, indtil man vælger outputsti

        elif path.lower().endswith(
                (".png", ".jpg", ".jpeg", ".webp", ".avif")):  # Hvis det er en enkel fil i følgende format
    ...
    def ask_output_folder(self):
        ...
        return folder if folder else self.create_unique_output_folder(
            self.default_output_folder)  # Hvis brugeren ikke selv vælger en mappe
    ...
    def create_unique_output_folder(self, base_folder):
        ...
        while os.path.exists(output_folder):
            output_folder = os.path.join(base_folder,
                                         f"PixelShifted_{counter}")  # Hvis der allerede findes en "PixelShifted" mappe, tæller counter up, så der ikke overwrites
            counter += 1
    ...
    def start_processing(self):
        ...
        if not os.path.exists(self.selected_path):
            Clock.schedule_once(lambda dt: setattr(self.ids.file_label, 'text',
                                                   f"Fejl, filen {os.path.basename(self.selected_path)} findes ikke! \n Er den blevet flyttet, eller slettet?"))
            return
    ...
    def process_image(self, image_path):
        try:
            ...
            if input_img.mode != "RGBA":  # Tjekker, om billedet er i alm. RGB farver og alpha (gennemsigtighed/styrke) via !=
                input_img = input_img.convert(
                    "RGBA")  # Hvis ikke, anvender vi .convert til at sørge for, at billedet er i det rigtige format
    ...
    def process_folder(self):
        try:
            files = [f for f in os.listdir(self.selected_path) if
                     f.lower().endswith(('.png', '.jpg', '.jpeg', '.webp', '.avif'))]
            ...
            for index, file in enumerate(
                    files):  # Hver fil bliver gemt som et index hvor enumerate sørger for at nummerere dem
                ...
                progress_value = int(
                    ((index + 1) / len(files)) * 100)  # Ud fra længden af files, altså, filerne i den valgte mappe
                # defineres procentdelen hver fil udgør
                Clock.schedule_once(lambda dt: setattr(self.ids.progress, 'value', progress_value),
                                    0)  # Opdaterer progressbar ud fra hvor langt i mappen programmet er

            Clock.schedule_once(lambda dt: self.update_file_info(self.selected_path, self.output_folder),
                                0)  # Når færdig, opdateres update_file_info
